      ...
      if (msgType === 'ai') {
        ...
        if (Array.isArray(msg.content)) {
          for (const block of msg.content) {
            if ((block.type === 'server_tool_use' || block.type === 'server_tool_call') && block.name === 'web_search') {
              let query = block.args?.query                        // LangChain-transformed
                || (typeof block.input === 'string' ? block.input : block.input?.query)  // raw Anthropic
                || '';
              if (!query && typeof block.partial_json === 'string') {
                try { query = JSON.parse(block.partial_json).query || ''; } catch {}
              }
              if (query) console.log(`[chatStream] Web search query: "${query}"`);
            }
          }
        }
